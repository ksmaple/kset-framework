...
- **版本**：子模块版本由根 `kset-boot-parent` / `kset-framework` BOM 统一管理，勿在子模块随意覆盖 Spring Boot 主版本。
- **测试依赖**：`spring-boot-starter-test` 由 `kset-boot-parent` 以 `test` scope 统一继承；子模块勿重复声明，新增测试能力（如 Testcontainers）在 parent 集中维护。
- **Lombok**：由 `kset-boot-parent` 以 `provided` 继承；Gateway 等模块若 IDE 未识别可显式声明同坐标（`provided`，不传递下游）。子模块勿用 `optional` 覆盖。
