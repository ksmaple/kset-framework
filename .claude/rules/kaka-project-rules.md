...
R009: AI 改码与 Git 操作禁止使用 git worktree，须在当前工作树内完成（分支策略见 R039）
...
## Git 工作树与分支（R009、R039）

R039: AI 会话禁止创建、切换或依赖 git worktree
R039a: 改码前须确认当前工作树路径与目标仓库一致
R039b: 改码前须检查当前分支并优先复用当前分支
R039c: 须切换或新建分支时必须先确认当前工作区无未处理变更
R039d: 须新建分支时，分支名须可识别本次任务并符合 `kaka-util-git-commit` 分支命名
R039e: 禁止为规避当前工作区变更而新建 worktree
R039f: 禁止未经用户明确要求向默认集成分支直接提交或合并会话变更
R039g: 非 git 仓库改码须先向用户说明无法执行分支保护
R039h: git commit、push 及提交分支策略以 `kaka-util-git-commit` 为准，与本节当前工作树规则互补
