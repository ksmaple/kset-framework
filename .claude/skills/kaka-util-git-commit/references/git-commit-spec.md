...
R039: Step 3 **开头**先据路径列表得到 `{feature-slug}`、`{author-slug}`（R043–R044），再计算 `<target>`；禁止为分支判定读 diff
...
R042: 禁止以 `dev`、`develop` 作为分支名基点；功能分支须以 `feat/` 为前缀
...
R044a: `{version}` 段可选，来自项目版本标签或 `pom.xml`/`package.json`/`build.gradle` 中的版本号，归一化为 `[a-z0-9.-]{1,16}`；无版本时省略该段
R045: `{feature-slug}` ≤28 字符、≤2 个英文词、kebab-case；**仅**据路径列表推断，规则见 [minimal-commit-inference.md](minimal-commit-inference.md)
R046: 大改动判定：变更文件数 ≥12；或 `git diff --shortstat` 的 insertions+deletions ≥400（仅统计，禁止读 patch）
R047: 分支尾 `{tail}`：有版本时大改动为 `dev-{version}-{yyyyMMdd}-{NN}-{feature-slug}-{author-slug}`，否则 `dev-{yyyyMMdd}-{NN}-{feature-slug}-{author-slug}`；非大改动时去掉 `{NN}-`
R048: 完整目标分支 `<target>` = `feat/{tail}`；version 省略时该段连同其前导 `-` 一并省略

### 当前分支解析（R049a 用）

R049p: 当前分支名匹配 `feat/dev(-{version})?-{yyyyMMdd}(-{NN})?-{feature-slug}-{author-slug}` 时解析：
R049p-1: `{author-slug}` = 最后一段（须与 R044 一致）
R049p-2: 去掉前缀 `feat/dev-`、可选版本段、8 位日期、可选 2 位 `{NN}` 及紧随其后的 `-` 后，余下去掉末段即为分支内 `{feature-slug}`（kebab-case，可与路径推断结果比对）
R049p-3: 非 `feat/dev-*` 形态（如 `main`、`feat/foo`、`dev-*`）视为无解析结果，不走 R049a
...
R049d: **切换同功能异名**：R049a–c 未命中，且 `git branch` / `git branch -r` 存在 `feat/dev(-{version})?-*-{feature-slug}-{author-slug}`（feature、author 与本次推断一致，日期或 `{NN}` 可不同）→ `git checkout` 该分支（多个时取本地最近更新者；仍禁止 `-b`）
