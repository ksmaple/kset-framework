...
R012: 编译验证仅在用户、任务或 CI 显式要求时执行，且默认只执行 `mvn -q -DskipTests compile` 的 compile-only 形态
R013: 后端工程编译等较长耗时命令须作为可选外部验证
R014: 文件逻辑交付不得被默认编译验证阻塞
...
Step 4: 完成文件逻辑自查并输出变更摘要。仅用户、任务或 CI 显式要求时执行 `mvn -q -DskipTests compile` 的最小 compile-only 命令，失败则交 `kset-framework-fixer`；显式要求测试时才执行 `mvn -q test`，测试形态见 test-execution-spec
