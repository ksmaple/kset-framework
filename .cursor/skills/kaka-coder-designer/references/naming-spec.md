...
N029: 时刻语义字段统一 `{action}At`（如 createdAt、expireAt）；纯日期语义 `{action}Date`；禁止裸 time/date/timestamp 作字段名；跨层键名见 N025 与 api/frontend 域 DateTime 映射
