...
> 自 `kset-comm` 更名为 `kset-framework`：根聚合 `artifactId`、BOM `kset-dependencies`、Boot parent `kset-boot-parent`、非 Boot parent `kset-parent`、Starter `kset-starter-*`；子模块 `kset-common` 名称不变。

## 模块结构

```
kset-framework/
├── kset-dependencies/        # 版本 BOM（Boot 3.5.14 / SC 2025.0.2 / SCA 2025.0.0.0 / Dubbo 3.3.6）
├── kset-parent/              # 非 Spring Boot 体系 parent
├── kset-boot-parent/         # Spring Boot 体系 parent（面向 kset-starter-*）
...
| 文档 | 说明 |
|------|------|
| [kset-dependencies/README.md](kset-dependencies/README.md) | 统一版本 BOM：Spring Boot / Cloud / Alibaba / Dubbo / 第三方库 / KSet 模块 |
| [kset-parent/README.md](kset-parent/README.md) | 非 Spring Boot 体系 parent：Java 21、编码、编译插件、通用测试依赖 |
| [kset-boot-parent/README.md](kset-boot-parent/README.md) | Spring Boot 体系 parent：版本基线与发布 |
...
|------|------------|----------|---------------|
| 版本基线 | `kset-boot-parent` / `kset-parent` | Maven parent | 统一 Java 21、UTF-8、Boot/Cloud/Alibaba/Dubbo 等版本 |
...
Spring 生态版本以 `kset-dependencies/pom.xml` 与 `kset-boot-parent/pom.xml` 为准：Spring Boot 3.5.x 对齐 Spring Cloud 2025.0.x，Spring Cloud Alibaba 使用 2025.0.0.0 版本线。

### 依赖分层

| 层级 | 模块 | 说明 |
|------|------|------|
| BOM | `kset-dependencies` | 锁定全量三方版本 |
...
若未使用 KSet Starter、仅导入 `kset-dependencies` 或继承 `kset-parent`，可按需从 BOM 引用（**无需写 version**）：
...
更多版本见 `kset-dependencies/pom.xml` 中 `dependencyManagement`。
...
| **编译** | `--release 21` | 由 `kset-boot-parent` / `kset-parent` 配置，含 `-parameters`（Spring / Dubbo 需要） |
...
Spring Boot 业务服务继承 `kset-boot-parent`：
...
### kset-boot-parent：Spring Boot 版本与构建基线
